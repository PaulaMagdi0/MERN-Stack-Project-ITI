    ...
    try {
      ...
      setDashboardStats({
        ...
        ratingDistribution: {
          ...
          data: [
            books.filter((book) => Number.parseInt(book.releaseDate) < 1900).length,
            books.filter(
              (book) => Number.parseInt(book.releaseDate) >= 1900 && Number.parseInt(book.releaseDate) < 2000,
            ).length,
            books.filter((book) => Number.parseInt(book.releaseDate) >= 2000).length,
          ],
        },
      })
  ...
  return (
    <DashboardContainer>
      <Sidebar>
        ...
        <NavItem active={activeTab === "manage-books"} onClick={() => setActiveTab("manage-books")}>
          <FaBook /> Manage Books
        </NavItem>
        <NavItem active={activeTab === "manage-authors"} onClick={() => setActiveTab("manage-authors")}>
          <FaUser /> Manage Authors
        </NavItem>
        <NavItem active={activeTab === "tables"} onClick={() => setActiveTab("tables")}>
          <FaChartBar /> Tables
        </NavItem>
        <NavItem active={activeTab === "charts"} onClick={() => setActiveTab("charts")}>
          <FaChartBar /> Charts
        </NavItem>
        <NavItem active={activeTab === "admin-registration"} onClick={() => setActiveTab("admin-registration")}>
          <FaUserShield /> Admin Registration
        </NavItem>
      </Sidebar>
      <MainContent>
        {activeTab === "overview" && <OverviewSection stats={dashboardStats} loading={loading} />}
        {activeTab === "manage-books" && <ManageBooks />}
        {activeTab === "manage-authors" && <ManageAuthors />}
        {activeTab === "tables" && (
          <TablesSection
        ...
        {activeTab === "charts" && <ChartsSection stats={dashboardStats} loading={loading} />}
        {activeTab === "admin-registration" && <AdminRegistrationSection />}
      </MainContent>
    </DashboardContainer>
  )
}
  ...
  <Section>
    ...
    {loading ? (
      <StatsGrid>
        {[...Array(4)].map((_, index) => (
          <StatCard key={index}>
            <SkeletonLoader height="48px" marginBottom="0" />
          </StatCard>
        ))}
      </StatsGrid>
    ) : (
      <>
        ...
        <ChartContainer>
          ...
          <ChartWrapper>
            {loading ? (
              <SkeletonLoader height="300px" marginBottom="0" />
            ) : (
              <Line
                data={{
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setTotalPagesBooks(booksData.totalPages)
        setPopularAuthors(authorsData.authors)
        setTotalPagesAuthors(authorsData.totalPages)
      } catch (error) {
        console.error("Error fetching data:", error)
      } finally {
        setLoading(false)
    ...
    fetchData()
  }, [currentPageBooks, currentPageAuthors])
    ...
    <Section>
      ...
      <TablesContainer>
        ...
        <TableWrapper>
          ...
          {loading ? (
            <SkeletonLoader lines={5} height="40px" marginBottom="10px" />
          ) : (
            <Table>
              <thead>
                <tr>
          ...
          {totalPagesBooks > 1 && (
            <PaginationContainer>
              ...
              <PageInfo>
                {currentPageBooks} / {totalPagesBooks}
              </PageInfo>
              <PaginationButton
                onClick={() => handlePageChangeBooks(currentPageBooks + 1)}
        ...
        <TableWrapper>
          ...
          {totalPagesAuthors > 1 && (
            <PaginationContainer>
              ...
              <PageInfo>
                {currentPageAuthors} / {totalPagesAuthors}
              </PageInfo>
              <PaginationButton
                onClick={() => handlePageChangeAuthors(currentPageAuthors + 1)}
...
TablesSection.propTypes = {
  popularBooks: PropTypes.arrayOf(
    PropTypes.shape({
  ...
  popularAuthors: PropTypes.arrayOf(
    PropTypes.shape({
      id: PropTypes.string.isRequired,
      name: PropTypes.string.isRequired,
